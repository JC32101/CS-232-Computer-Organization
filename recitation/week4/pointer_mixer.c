...
void strlen_vs_sizeof() {
    char str[]="Hello!"; //store in STACK
    char str2[8]="Hello!"; //store as constant CANT BE MODIFY!!
    char * s = str;
...
void simple_double_array() {
    int darray[][4] = { {1, 2, 3, 4},
                       ...
                       {13, 14, 15, 16}};
    int i,j;
    printf("*(*(darray+3)+2) = %d\n", *(*(darray+3)+2)); //*(*(darray+3)+2) = 15
}
// ----------------------------------------------------------------------------
void string_double_array_pointer_array() {
    //these are two strings
    char str1[] = "This is a locust tree"; //str1 is a statically alloc-ed array //string literal IN STACK
    char * str2 = "This is also a locust tree"; //str2 is a pointer to char //in Stacj but point on global fragment 
    ...
    //String1  would be at the higher addres because it was declare first and pushed into the stack first	
...
void string_equal() {
    ...
    char * s4 = "Pace";
    if(s1 == s2){
        printf("Go Pace!\n"); //it would not print because it had different addresses
    }else if(s3 == s4) {
        printf("Beat CUNY!\n"); //PRINTS 
    }else {
        printf("Crash SUNY!\n");
...
void trace_structs_pointers()
{
    ...
    my_stuff.a = &temp;
    my_stuff.b = 1;
    printf("a=%d b=%d\n", *(my_stuff.a), my_stuff.b); //a=0 b=1

    foo(my_stuff);
    printf("a=%d b=%d\n", *(my_stuff.a), my_stuff.b);//a=2 b=1

    bar(&my_stuff);
    printf("a=%d b=%d\n", *(my_stuff.a), my_stuff.b);//a=4 b=5
}
...
void readonly_vs_stack() {
  ...
  //accessing what we shouldn't, you cannot access one part
  //FIX
  char * str2 = (char*)malloc(100); 
  strcpy(str2, "This is also a locust tree");

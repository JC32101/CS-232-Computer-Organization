...
int ll_has_cycle(node *head) {
  ...
  while(hare && hare->next){ //not NULL
    //Advance hare by two nodes. 
    //Advance tortoise by one node
		hare = hare->next->next;
      ...
      //If tortoise and hare point to the same node =  list is cyclic
			return head;
		}
	}
  //If this is not possible = found the end of the list = acyclic list
	return NULL;
